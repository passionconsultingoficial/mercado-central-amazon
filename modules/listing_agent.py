...
def analisar_imagem_visuo_computacional(image_bytes: bytes, mime_type: str, api_key: str) -> str:
    """Análise multimodal via Claude Vision para extração dinâmica do nome do produto."""
                            ...
                            "content": [
                                ...
                                },
                                {
                                    ...
                                    "text": (
                                        ...
                                        "Retorne APENAS o nome comercial exato do produto em português do Brasil (2 a 5 palavras). "
...
def buscar_melhor_vendedor_organico_amazon_br(query: str) -> dict:
    """
    Filtra anúncios patrocinados (Ads) e extrai o primeiro resultado orgânico real da Amazon BR.
    """
    headers = {
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            for prod in produtos:
                # IGNORA ANÚNCIOS PATROCINADOS / ADS
                is_sponsored = (
...
def gerar_anuncio_nativo_fallback_dinamico(termo_exibicao: str, dados_lider: dict) -> str:
    """
    Gera o anúncio 100% dinâmico extraindo apenas os termos da consulta fornecida,
    garantindo que NENHUM nome de produto fique hardcoded no código.
    """
    words = [w.title() for w in re.findall(r'\w+', termo_exibicao) if len(w) > 1]
    prod_base = " ".join(words) if words else termo_exibicao.title()

    # Monta títulos dinâmicos baseados estritamente na consulta do produto
    t_a = f"{prod_base} Reforçado Modelo Prático Alta Durabilidade Uso Diário"
    if len(t_a) > 75:
        corte = t_a[:75]
        t_a = corte.rsplit(" ", 1)[0] if " " in corte else corte
    
    t_b = f"{prod_base} Multiuso Confortável Formato Ergonômico Prático"
    if len(t_b) > 75:
        corte = t_b[:75]
        t_b = corte.rsplit(" ", 1)[0] if " " in corte else corte

    # Gera palavras-chave backend puramente dinâmicas sem repetir o título
    ...
    atributos_dinamicos = [
        ...
        "eficiente", "resistentete", "original", "uso", "diario", "conforto"
    ]
    ...
    return (
        f"### 📋 Relatório Comparativo: Nosso Produto vs. Líder de Vendas Orgânico\n\n"
        f"| Métrica / Atributo | Concorrente Líder (Amazon BR) | Nossa Estratégia de Produto |\n"
        f"| :--- | :--- | :--- |\n"
        ...
        f"| **Preço Estimado** | `{dados_lider['preco_lider']}` | Posicionamento Competitivo |\n"
        f"| **Pontos Fortes do Líder** | Posição orgânica consolidada e volume acumulado de vendas. | Foco em diferenciação técnica, clareza de atributos e copy persuasiva. |\n"
        f"| **Dores Mapeadas nos Reviews** | Queixas frequentes sobre fragilidade do acabamento e expectativa frustrada de tamanho. | Neutralização explícita nas primeiras linhas dos Bullets e imagens técnicas. |\n\n"
        f"---\n\n"
        ...
        f"Proporcione a combinação perfeita de eficiência, durabilidade e praticidade com o {prod_base}. "
...
def processar_e_gerar_markdown(termo_entrada: str) -> str:
    ...
    return links_md + gerar_anuncio_nativo_fallback_dinamico(termo_exibicao, dados_lider)
